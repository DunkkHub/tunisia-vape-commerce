...
`20260804120000_manual_courier_operations` adds optional company/WhatsApp contact data, availability, bounded capacity, a bounded default internal courier fee, and a courier-to-delivery-zone join with an optional bounded internal zone fee. Existing couriers remain available and retain unrestricted coverage until an operator explicitly assigns zone rows, so the additive migration does not invalidate historical assignments. Internal courier costs are distinct from `DeliveryRate.feeMillimes`; they never replace the server-calculated customer fee or an order's immutable delivery snapshot. The migration creates no courier, coverage, credential, delivery rate, assignment, or customer data.

`20260804130000_collection_discrepancy_scope` adds nullable foreign-key links from a collection-level `CashDiscrepancy` and its `CashReconciliationEvent` rows to the exact `CashCollection`, plus scope checks that reject orphan/hybrid discrepancy coordinates and remittance/collection hybrid events. It first backfills a fully cross-checked, unique `DISCREPANCY_OPENED` event coordinate; conflicting event metadata remains unlinked, while rows with no such event may use a unique order-and-amount fallback. Ambiguous legacy rows remain unlinked so reconciliation fails closed instead of guessing. A successful exact second count never rewrites `CashCollection.collectedMillimes`: it appends a linked `ADJUSTMENT_RECORDED` event, closes the discrepancy under dual control, and updates only workflow projections. A write-off closes the investigation but leaves the order payment projection discrepant. Remittance allocation uses the recorded amount for a write-off and the expected amount only after an exact resolved adjustment.

`20260811170000_product_image_renditions` adds one immutable metadata row per image, rendition name, negotiated format, and encoder profile version. The row stores integer byte size, SHA-256, and bounded dimensions; object keys remain server-derived. New image rows and all eight rendition records commit together after object staging. Existing images remain readable through a serial, checksum-verified first-read backfill; a conflicting cached object fails closed instead of being trusted or overwritten.
