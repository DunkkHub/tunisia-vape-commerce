...
| OAuth interception, replay or mis-link  | Customer takeover or provider identity attached to wrong user  | Exact customer callback; state, nonce and PKCE; cookie binding; official signature/claim checks; one-time Redis state; verified-email/password link policy   | Callback replay, claim and account-link tests    |
...
Google OAuth terminates only under the customer controller. Its callback is the sole cross-site navigation exception and is constrained to an exact GET path plus browser Fetch Metadata. Authorization state is one-time, encrypted, cookie-bound and short-lived; nonce and PKCE bind the verified Google response to the initiating browser. Persistent external identity rows can reference only `CustomerProfile`, so provider authentication has no route to an administrator principal. Query-bearing callback URLs are removed from structured request logs before serialization.
