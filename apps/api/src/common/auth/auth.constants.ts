...
export const cookieNames = (production: boolean) => ({
  ...
  customerGoogleState: production
    ? '__Host-vape_customer_google_state'
    : 'vape_customer_google_state',
  customerGoogleOnboarding: production
    ? '__Host-vape_customer_google_onboarding'
...
export const oauthCookieOptions = (production: boolean, maxAge: number) => ({
  httpOnly: true,
  maxAge,
  path: '/',
  sameSite: 'lax' as const,
